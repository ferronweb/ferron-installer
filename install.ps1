...
# Print the splash
Write-Host '***************************************'
Write-Host '**Ferron installer for Windows Server**'
Write-Host '***************************************'
Write-Host ''
...
if ($installType -eq "manual") {
  ...
  $ferronVersion = (Invoke-RestMethod -Uri "https://downloads.ferronweb.org/latest.ferron").Trim()
...
$ferronYamlConfig = @"
global:
  wwwroot: `"$env:SYSTEMDRIVE\\ferron\\wwwroot`"
  logFilePath: `"$env:SYSTEMDRIVE\\ferron\\logs\\access.log`"
  errorLogFilePath: `"$env:SYSTEMDRIVE\\ferron\\logs\\error.log`"
"@
$ferronYamlConfig | Out-File -FilePath "$env:SYSTEMDRIVE\ferron\ferron.yaml" -Encoding ascii  
...
<service>
  ...
  <arguments>-c $env:SYSTEMDRIVE\ferron\ferron.yaml</arguments>
  <log mode=`"none`"></log>
</service>
"@
